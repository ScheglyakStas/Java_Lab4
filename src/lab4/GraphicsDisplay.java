...
public class GraphicsDisplay extends JPanel {
    ...
    public GraphicsDisplay() {
        ...
        float[] dashArray=new float[]{20,5,20,5,20,5,
                                        5,5,5,5,5,5};
        graphicsStroke = new BasicStroke(2.0f, BasicStroke.CAP_BUTT,
                BasicStroke.JOIN_ROUND, 10.0f, dashArray, 0.0f);
    ...
    public boolean checkDigits(Double[] point)
    {
        ...
        while(y>0.0&&result) {
            ...
            if ((Math.floor(y) > Math.floor((y-Math.floor(y))*10))
            && (y-Math.floor(y)>0))
            {
                result=false;
            }
...
его центра
    и угла прямоугольника, в который он вписан */
            ...
            canvas.draw(marker);
            canvas.draw(new Line2D.Double(xyToPoint(point[0]-0.05, point[1]-0.05),
                    xyToPoint(point[0]+0.05, point[1]+0.05)));
            canvas.draw(new Line2D.Double(xyToPoint(point[0]-0.05, point[1]+0.05),
                    xyToPoint(point[0]+0.05, point[1]-0.05)));// Начертить контур маркера
